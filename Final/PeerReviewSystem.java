import java.util.ArrayList;
import java.util.HashMap;
import java.util.Map;

public class PeerReviewSystem {
    public ArrayList<Assignment> assignments = new ArrayList<>();
    public Map<String,Double> schoolStrategy = new HashMap<String,Double>();
    public ArrayList<Student> students = new ArrayList<>();
    
    public void printRubric(){
        
    }
    public void averageCriterion(String assignmentID){

    }
    public void calculateScore(String AID,String SIO,RankingStrategy rs){

    }
    public void findStrength(String AID,String SIO,RankingStrategy rs){

    }
    public void findWeakness(String AID,String SIO,RankingStrategy rs){

    }
}
